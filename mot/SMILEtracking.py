...
def main(seq):
    ...
    tracker = Modified_SMILEtrack.Modified_Tracker()
 ...
 #   mot_feat_data = {}
    ...
    with open(save_path, 'w') as final_result:
        for frame_number, frame_img in enumerate(sorted_source_files): 
            ...
            if os.path.exists(det_file_path):
                with open(det_file_path, 'r') as f:
                    ...
                    for line in lines:
                        ...
                        bbox_temp = (x1,y1,x2,y2) # careful about this !!
            ...
            tracker.update(bbox,confidence,feats,class_ids,frame_img)
            for index, track in enumerate(tracker.tracks):
                ...
                output_line = f"{frame_number} {track_id} {x1} {y1} {x2} {y2} {cls}\n"
