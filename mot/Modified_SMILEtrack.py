from SMILEtrack.SMILEtrack_Official.tracker.mc_SMILEtrack import SMILEtrack
from SMILEtrack.SMILEtrack_Official.yolov7.utils.torch_utils import select_device
import argparse
import torch
import numpy as np
...
opt.jde = False
opt.exist_ok = True
device = opt.device # is a empty string
...
class Modified_Tracker:
    ...
    def __init__(self):
        self.tracker = SMILEtrack(opt,frame_rate=10) # tracker initalized
    ...
    def update(self,bboxs,confidence,feats,class_ids,img):
        ...
        for bbox_id, bbox in enumerate(bboxs):
            ...
            det = np.array([x, y, w, h, confidence[bbox_id], class_ids[bbox_id]])
