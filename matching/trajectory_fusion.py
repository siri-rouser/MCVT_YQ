...
def parse_pt(pt_file,zones):
    ...
    mot_list = dict()
    for line in lines:
        # data loading through the .pkl file
    ...
    return mot_list # it contains the BBOX(with track_id) is in which zone, with track_id and frame to be the key
    ...
    for sname in scene_name:
        with open(opj(timestamp_dir, sname + '.txt')) as f:
            ...
            for line in lines:
                ...
                cid = int(line[0][2:])# it means the first element 'c041' e.g. and all elements after the second elements, 41 in here.
                bias = float(line[1])
...
if __name__ == '__main__':
    ...
    cid_bias = parse_bias(cfg.CID_BIAS_DIR, scene_name) # A dict() contains id and 
    # !!! Be careful about the bbox information !!!(seem to be x1,y1,x2,y2) in here
    if not os.path.isdir(save_dir):
        os.makedirs(save_dir)
    cam_paths = os.listdir(data_dir) # It lists the folder name rather than the path
    cam_paths = list(filter(lambda x: 'c' in x, cam_paths))
    cam_paths.sort()
    zones = zone()
    # print(cam_paths) ['c041', 'c042', 'c043', 'c044', 'c045', 'c046']

    for cam_path in cam_paths:
        print('processing {}...'.format(cam_path))
        cid = int(cam_path[-3:]) # it is camera ID in here [-3:] = [2:]
        f_w = open(opj(save_dir, '{}.pkl'.format(cam_path)), 'wb') # ./exp/viz/test/S06/trajectory/c041.pkl
        cur_bias = cid_bias[cid] # All 0 0 0 0 0 in S06
        ...
        print(new_mot_path)
        zones.set_cam(cid)
        mot_list = parse_pt(mot_path,zones)# return info contain single camera trajectory with ZONE information
        ...
        out_new_mot(mot_list, new_mot_path) # save all results here
        tid_data = dict()
        ...
        for tid in mot_list:
            ...
            feature_list = [tracklet[f]['feat'] for f in frame_list if (tracklet[f]['bbox'][3]-tracklet[f]['bbox'][1])*(tracklet[f]['bbox'][2]-tracklet[f]['bbox'][0])>2000] # If the cropped img area is bigger than 2000?
            if len(feature_list)<2:
                feature_list = [tracklet[f]['feat'] for f in frame_list]
            io_time = [cur_bias + frame_list[0] / 10., cur_bias + frame_list[-1] / 10.] # 10fps
            ...
            } # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Information for later matching is contained in here.
