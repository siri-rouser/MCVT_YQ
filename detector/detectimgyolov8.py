...
# Load a model
model = YOLO("/data/yl4300/AIC22-MCVT/detector/yolov8x.pt")  # load an official model
vehicles = [2, 3, 5, 7]


for seq in seqs:
    #DONOT FORGET TO MODIFY THE PATH!!!!!!!!!!!!!!
    source = f'/data/yl4300/AIC22-MCVT/datasets/algorithm_r_result/detection/images/test/S06/{seq}/img1/'
    ##############################################

    # Directories
    # save_dir = Path(increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok))  # increment run
    save_txt = True
    save_dir = Path(Path('/data/yl4300/AIC22-MCVT/datasets/algorithm_results/detect_merge') / Path(seq))
    (save_dir / 'labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make dir

    save_img = True
    dataset = ultralytics.data.loaders.LoadImages(source, imgsz=1280) # Hope this would be useful
        ...
        for i, result in enumerate(result):  # detections per image
            ...
            if len(result): # if any vehicle is been detected here
                ...
                for res in result:
                    ...
                    # Write to file
                    if save_txt:
                        with open(txt_path + '.txt', 'a') as f: # a means append, what f.write in here will be appended into the end of the file
